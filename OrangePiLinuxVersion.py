...
import time
import importlib
# Installing openpyxl if not already
try:
	...
	try:
		import subprocess
		subprocess.check_call(['sudo', 'pip', 'install', 'openpyxl'])
		print("Done")
	except Exception as e:
		print("err")
		os.system("sudo pip install openpyxl")
import openpyxl
...
#                UI(file_name, date_today, workbook)
        ...
        if True ==1:
            new_dir = "/media/orangepi/STORE N GO/"  # replace with directory path of thumb drive
            folder = "BarcodeExcels/"  # replace with name of folder
            print(new_dir + folder)
            backup_path = "/home/orangepi/Desktop/BACKUP/"
            # branch path if there is no thumb drive inserted
            if not os.path.exists(new_dir):
                # branch if backup folder doesnt exist
               if not os.path.exists(backup_path):
                    os.makedirs(backup_path)
                    # working directory for files is on desktop
               new_dir = backup_path
            # completing path with folder
            # path if thumb drive is inserted but there is no folder on it
            elif not os.path.exists(new_dir + folder):
            	new_dir += folder
            	os.makedirs(new_dir)
            else:
            	new_dir += folder
...
def main():
    def funct(barcode):
        ...
        while barcode not in ["q", "Q", "Quit", "quit", "QUIT", "EXIT", "Exit",
                              "exit"]:  # terminate loop when 'q' is entered
    ...
    def restart():
        ...
        if decision in ("yes", "y", "no", "n", "again", "done"):
 
            while decision in ("yes", "y", "again"):
                main()
            time.sleep(1)
            print("\n\n\n\n     Closing Program...\n")
            time.sleep(2)
...
if __name__ == '__main__':
    try:
        ...
        log_directory = "/media/orangepi/STORE N GO/"
        log_folder = "ERROR_LOGS/"
        log_backup_path = "/home/orangepi/Desktop/BACKUP/ERROR_LOGS"
 
        if not os.path.exists(log_directory):
            if not os.path.exists(log_backup_path):
                os.makedirs(log_backup_path)
            final_log_dir = log_backup_path
        elif not os.path.exists(log_directory + log_folder):
            final_log_dir = (log_directory + log_folder)
        ...
        main()
            ...
            while os.path.exists(final_log_dir + "/" + log_name):
                ...
                log_name += ".txt"
            log_file_path = os.path.join(final_log_dir, log_name)
